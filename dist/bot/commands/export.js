...
exports.setExportConnection = setExportConnection;
exports.exportCmd = exportCmd;
const grammy_1 = require("grammy");
const state_1 = require("../../state");
const orders_1 = require("../../ctrader/orders");
let connection = null;
function setExportConnection(conn) {
    connection = conn;
}
...
function parseDateArg(s, endOfDay) {
    ...
    return Date.UTC(+y, +mo - 1, +d, hasTime ? +hh : endOfDay ? 23 : 0, hasTime ? +mm : endOfDay ? 59 : 0, hasTime ? 0 : endOfDay ? 59 : 0);
}
async function exportCmd(ctx) {
    ...
    for (const d of closing) {
        ...
        const spec = await (0, orders_1.getSymbolSpec)(symId);
    ...
    await ctx.replyWithDocument(new grammy_1.InputFile(Buffer.from(json, "utf-8"), fname), { caption });
}
