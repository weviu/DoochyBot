...
exports.setLivePriceConnection = setLivePriceConnection;
exports.subscribeSpots = subscribeSpots;
exports.subscribeOpenPositions = subscribeOpenPositions;
exports.getMarkPrice = getMarkPrice;
exports.hasLiveQuote = hasLiveQuote;
...
async function subscribeSpots(symbolIds) {
    if (!connection)
        return;
...
async function subscribeOpenPositions() {
    const ids = [...new Set([...state_1.state.positions.values()]
            .map((p) => state_1.state.symbolMap.get(p.symbol))
...
function getMarkPrice(symbol, direction) {
