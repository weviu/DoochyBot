...
function notifyFill(kind, signal, lots, entry, positionId, riskUsd, sl, tp) {
    if (!state_1.state.settings.notifyFills)
        return;
    const slPct = (0, state_1.slPctFor)(signal.symbol);
    const tpPct = (0, state_1.tpPctFor)(signal.symbol);
...
async function reconcilePositions() {
    ...
    try {
        ...
        for (const p of positions) {
            ...
            const reTpPct = (0, state_1.tpPctFor)(posSlot.symbol);
...
async function executeSignal(signal) {
    ...
    if (state_1.state.settings.marginAware) {
    ...
    try {
        ...
        console.log(`[MARGIN-DIAG] ${signal.direction} ${signal.symbol}: needs ~$${dm !== null ? dm.toFixed(2) : "?"} margin, notional ~$${notional.toFixed(0)} (effective leverage ~1:${lev}), balance $${state_1.state.accountInfo.balance.toFixed(2)}, open positions ${state_1.state.positions.size}`);
    }
